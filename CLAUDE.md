...
The app is driven by a top-level `st.radio` at line ~2798 in `app.py` with five modes:
...
`desenhar_placar()` (line ~426 in `app.py`) is the main PIL composition function — it places team badges, score text, and scorer names onto a template PNG and is ~190 lines long.
...
- `escudos-ucl/`, `escudos-uel/`, `escudos-uecl/` — badge PNGs for European club competitors. **English clubs** in these competitions resolve their badge from `escudos-pl/` instead (handled in `obter_escudo_path()`, line ~393 in `app.py`, via the `INGLES_UCL` / `INGLES_UEL` / `INGLES_UECL` lists)
